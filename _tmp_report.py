  ...
  <body class="min-h-screen">
    <div class="max-w-7xl mx-auto px-4 py-6">
      ...
      <div class="flex flex-wrap gap-3 mb-6 text-sm">
        <a href="index.csv" class="px-3 py-1 rounded bg-gray-800 border border-gray-700 hover:bg-gray-700">Download Index (CSV)</a>
        <a href="weekly-latest.md" class="px-3 py-1 rounded bg-gray-800 border border-gray-700 hover:bg-gray-700">Weekly Digest (Markdown)</a>
      </div>
      <div class="card mb-6">
    ...
    <script>
      ...
      function renderCharts(rep) {
        ...
        const items = rep.top_items_detail && rep.top_items_detail.length ? rep.top_items_detail : rep.top_items || [];
        el.innerHTML = items.map(t => (
          `<div class='mb-3'>
            <div class='font-medium'>${t.title}</div>
            <div class='text-sm text-gray-400 mb-1'>score: ${Number((t.overall||t.score||0)).toFixed(3)}</div>
            ${t.tldr ? `<div class='text-sm mb-1'>${t.tldr}</div>` : ''}
            ${(t.pillars||[]).map(p=>`<span class='pill'>${p}</span>`).join('')}
      ...
      async function init() {
        ...
        renderHistory(hist);
        // Last updated from history (if present)
        try {
        ...
        const data = (rep.top_items_detail||rep.top_items||[]).map(t=>({title:t.title,url:t.url,text:(t.tldr||t.title||''),pillars:t.pillars||[]})).concat((rep.recommendations||[]).map(r=>({title:r.title,url:r.url,text:r.summary||r.title,pillars:r.pillars||[], source:''})));
        function doSearch(){
