...
# 1) Ensure history entries include a status field (success)
s = re.sub(r"hist\.append\(\{([\s\S]*?)\}\)", lambda m: (
    (lambda body: 'hist.append({' + (body + ("\n        \"status\": \"success\"," if '"status"' not in body else '')) + '\n    })')
)(m.group(1)), s, count=1)

# 2) In items.json build, include evidence verdict/confidence (if not already)
# Find the items.json generation block
items_block_pat = re.compile(r"\(out_dir / 'items\.json'\)\.write_text\([\s\S]*?\)")
if not items_block_pat.search(s):
    # If injection hasn’t been done yet, do nothing silently
    pass
else:
    # Enhance earlier CSV loop to include verdict/confidence
    s = re.sub(r"items\.append\(item\)\n\s*\}\n\s*except Exception:",
               """
                    # attach evidence verdict/confidence when present
                    try:
                        month = f"{iid[:4]}-{iid[4:6]}"
                        ev = (out_dir.parent / 'items' / month / iid / 'evidence.json')
                        if ev.exists():
                            evj = json.loads(ev.read_text(encoding='utf-8'))
                            item['verdict'] = (evj.get('verdict') or '').lower()
                            if evj.get('confidence') is not None:
                                try:
                                    item['confidence'] = float(evj.get('confidence'))
                                except Exception:
                                    pass
                    except Exception:
                        pass
...
# 3) Replace HTML template with new layout 
html_pat = re.compile(r"html = \"\"\"([\s\S]*?)\"\"\"")
new_html = r'''html = """
<!doctype html>
...
s = html_pat.sub(new_html, s, count=1)

# 4) Ensure the index.html write remains
if 'dashboard.html' in s and '(out_dir / "index.html").write_text(html' not in s:
    s = s.replace('(out_dir / "dashboard.html").write_text(html, encoding="utf-8")',
